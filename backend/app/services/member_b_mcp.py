...
from app.tools.member_b_tools import dispatch_local_tool, member_b_tool_names
...
class MemberBMCPClient:
    def __init__(
        ...
        self.local_fallback = False if local_fallback is None else local_fallback
    ...
    def call_tool(self, name: str, arguments: dict[str, Any]) -> MCPToolResult:
        if name not in member_b_tool_names():
            raise ValueError(f"Unknown Member B tool: {name}")

        started = perf_counter()
        try:
            data = self._remote_caller(name, arguments)
            return MCPToolResult(data=data, transport="mcp", duration_ms=_elapsed_ms(started))
    ...
    def _call_remote(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        raise RuntimeError("MemberBMCPClient.call_tool must run outside an active event loop")
